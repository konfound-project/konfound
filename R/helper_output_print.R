...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat(paste0("The estimated effect is ", round(est_eff, 3), ", and specified threshold for inference is", round(eff_thr, 3), "."))
      cat("\n")
      cat(paste0("To reach that threshold, ", round(sustain, 3), "% of the (", round(est_eff, 3), ") estimate"))
      cat("\n")
      cat(paste0("would have to be due to bias. This implies that to sustain an inference"))
      cat("\n")
