...
if (requireNamespace("htmltools", quietly = TRUE)) {
  ...
  if (index == "IT") { 
    ...
    if (abs(obs_r) > abs(critical_r) & obs_r > 0) {
      ...
      cat("\n")
      cat(paste0("on a correlation of ", -r_con, " with the outcome and at ", r_con))
      cat("\n")
      cat("with the predictor of interest (conditioning on all observed covariates")
      cat("\n")
      cat(paste0("in the model; signs are interchangable) based on a threshold of ", round(critical_r, 3)))
      cat("\n")
      cat("for statistical significance (alpha = ", alpha, ").\n", sep = "")
      cat("\n")
      cat("Correspondingly the impact of an omitted variable (as defined in Frank 2000) must be ") 
