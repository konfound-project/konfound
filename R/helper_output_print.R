...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("statistical significance with alpha= ", alpha, " ,", round(bias, 3), " % of the (", round(est_eff, 3), ")"))
      cat("\n")
      cat(paste0("estimate would have to be due to bias. This implies that", round(recase, 3)))
      cat("\n")
      cat("to change invalidate the inference one would expect to have to replace\n")
      cat(paste0(round(recase, 3), "observations with cases for which the treatment effect is 0 (RIR = ", round(recase, 3), ").\n"))
      cat("\n")
      ...
      cat("\n")
      cat(paste0("This implies that to invalidate the inference one would expect to have to replace "))
      cat("\n")
      cat(round(recase, 3), "observations with cases for which the treatment effect is ", nu, "(RIR = ", round(recase, 3),").\n")
      cat("\n")
      ...
      cat("\n")
      cat(paste0("threshold, ", round(bias, 3), "% of the (", round(est_eff, 3), ") estimate would have to be due to"))
      cat("\n")
      cat("bias. This implies that to sustain the inference on would expect to have")
      cat("\n")
      cat(paste0("to replace ", round(recase, 3), "observation with effect of", nu, "with cases with effect of ", round(beta_threshhold, 3), "(RIR = ", round(recase, 3), ")."))
      cat("\n")
    } else if ((abs(est_eff) < abs(beta_threshhold)) & is.na(eff_thr) == FALSE) {
      cat("TO SUSTAIN:\n", sep = "")
      cat("\n")
      ...
      cat("\n")
      cat(paste0("value of ", round(eff_thr, 3), ". To reach that threshold, ", round(bias, 3), "% of the (", round(est_eff, 3), ")"))
      cat("\n")
      cat(paste0("estimate would have to be due to bias. This implies that"))
      cat("\n")
      cat("to sustain the inference one would expect to have to replace ")
      cat("\n")
      cat(paste0(round(recase, 3), "observation with effect of ", nu, "with cases with effect of ", round(beta_threshhold, 3), " (RIR = ", round(recase, 3), ")."))
      cat("\n")
