...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("is ", round(eff_thr, 3), ". To invalidate the inference based on your estimate,"))
      cat("\n")
      cat(paste0(round(bias, 3), "% of the (", round(est_eff, 3), ") estimate would have to be due to bias"))
      cat("\n")
      cat(paste0("This implies that to invalidate the inference one would expect to have"))
      cat("\n")
      cat(paste0("to replace ", round(recase, 3), " (", round(bias, 3), "%) observations with cases for which"))
      cat("\n")
      cat(paste0("the treatment effect is ", nu, " (RIR = ", round(recase, 3), ").\n"))
      cat("\n") 
