...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("the threshold of ", round(beta_threshhold, 3), " for statistical significance with alpha= ", alpha, ", ", round(bias, 3), "%"))
      cat("\n")
      cat(paste0("of the (", round(est_eff, 3), ") estimate would have to be due to bias. This implies"))
      cat("\n")
      cat("that to invalidate the inference one would expect to have to replace")
      cat("\n")
      cat(paste0(round(recase, 3), " (", round(bias, 3), "%) observations with cases for which the treatment effect"))
      cat("\n")
      cat(paste0("is ", nu, " (RIR = ", round(recase, 3), ").\n"))
      cat("\n")    
