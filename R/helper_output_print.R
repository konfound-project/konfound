...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("The estimated effect is ", round(est_eff, 3), ". To invalidate the inference of"))
      cat("\n")
      cat(paste0("an effect using the threshold of ", round(beta_threshhold, 3), " for statistical significance"))
      cat("\n")
      cat(paste0("with alpha = ", alpha, ", ", round(bias, 3), "% of the (", round(est_eff, 3), ") estimate would"))
      cat("\n")
      cat(paste0("have to be due to bias. This implies that to invalidate the inference "))
      cat("\n")
      cat(paste0("one would expect to have to replace ", round(recase, 3), " (", round(bias, 3), "%) observations"\n"))
