...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("is ", round(beta_threshhold, 3), " (alpha = ", alpha, "). To reach that threshold, ", round(sustain, 3), "%"))
      cat("\n")
      cat(paste0("of the (", round(est_eff, 3), ") estimate would have to be due to bias. This implies to"))
      cat("\n")
      cat("to sustain an inference one would expect to have to replace ", round(recase, 3))
      cat("\n")
      cat(paste0("(", round(sustain, 3), "%) observations with effect of ", nu, " with cases with effect of"))
      cat("\n")
