...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("the inference one would expect to have to replace ", round(recase, 3), "(", round(bias, 3), "%)"))
      cat("\n")
      cat(paste0("observations with cases for which the treatment effect is ", nu, " (RIR = ", round(recase, 3), ").\n"))
      cat("\n")
      cat("\n") 
