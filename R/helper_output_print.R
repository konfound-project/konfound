...
output_print <- function(beta_diff, beta_threshhold, bias = NULL, sustain = NULL, nu, recase, obs_r, critical_r, r_con, non_linear) {
    ...
    if (abs(obs_r) > abs(critical_r)) {
        ...
        cat("Correspondingly the impact of an omitted variable (as defined in Frank 2000) must be", r_con, "*", r_con, "=", round(sqrt(r_con), 3), "to sustain an inference.\n") }
        ...
        cat("Correspondingly the impact of an omitted variable (as defined in Frank 2000) must be", r_con, "*", r_con, "=", round(sqrt(r_con), 3), "to invalidate an inference.\n") }
    else if (r_con == itcv) {
