...
output_print <- function(n_covariates,
  ...
  if (index == "RIR"){
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("the threshold of ", round(beta_threshhold, 3), " for statistical significance (with null hypothesis = ", nu))
      cat("\n")
      cat(paste0(" and alpha = ", alpha, "), ", round(bias, 3), "% of the (", round(est_eff, 3), ") estimate would have to be due to bias."))
      cat("\n")
      cat(paste0("This implies that to invalidate the inference one would expect to have to replace"))
      cat("\n")
      cat(paste0(round(recase, 3), " (", round(bias, 3), "%) observations with data points for which the effect is ", nu))
      cat("\n")
      cat(paste0("(RIR = ", round(recase, 3), ").\n"))
      cat("\n")
...
#}
  ...
  if (index == "IT") { 
    ...
    if (abs(obs_r) > abs(critical_r) & obs_r > 0) {
      ...
      cat("\n")
      cat(paste0("for a null hypothesis of an effect of nu (", nu, ") effect is based on a correlation of ", round(r_con, 3)))
      cat("\n")
      ...
      cat("\n")
      cat(paste0("invalidate an inference for a null hypothesis of an effect of nu (", nu, ") is based on"))
      cat("\n")
      ...
      cat("Correspondingly the impact of an omitted variable (as defined in Frank 2000) must be ")
      cat("\n")
      cat(-round(r_con, 3), " X ", round(r_con, 3), " = ", -round(r_con^2, 3), " to sustain an inference for a null hypothesis of an effect of nu (", nu, ").\n", sep = "")
    } else if (abs(obs_r) < abs(critical_r) & obs_r < 0) {
      cat("The maximum impact of an omitted variable to sustain an inference")
      cat("\n")
      cat(paste0("for a null hypothesis of an effect of nu (", nu, ") is based on a correlation of ", round(r_con, 3)))
      cat("\n")
      cat(paste0("with the outcome and ", round(r_con, 3), " with the predictor of interest (conditioning"))
      cat("\n")
