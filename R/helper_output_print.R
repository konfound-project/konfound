...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("using the threshold of ", round(beta_threshhold, 3), " for statistical significance with"))
      cat("\n")
      cat(paste0("alpha = ", alpha, ", ", round(bias, 3), "% of the (", round(est_eff, 3), ") estimate would have to be"))
      cat("\n")
      cat("due to bias. This implies that to invalidate the inference one would")
      cat("\n")
      cat(paste0("expect to have to replace ", round(recase, 3), " (", round(bias, 3), "%) observations with cases"))
      cat("\n")
      cat(paste0("with cases for which thetreatment effect is ", nu, " (RIR = ", round(recase, 3), ").\n"))
      cat("\n")    
      cat("\n")    
