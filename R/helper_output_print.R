...
output_print <- function(est_eff, beta_threshhold, bias = NULL, sustain = NULL, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index) {
  if (index == "RIR"){ 
    ...
    if ((abs(est_eff) > abs(beta_threshhold)) & is.na(eff_thr) == TRUE) {
      ...
      cat("\n")
      cat(paste0("is ", round(beta_threshhold, 3), " (alpha = ", alpha, "). To reach that threshold, ", round(sustain, 3), "% of the (", round(est_eff, 3), ")"))
      cat("\n")
      cat(paste0("estimate would have to be due to bias. This implies to sustain an inference"))
      cat("\n")
      cat(paste0("one would expect to have to replace", round(recase, 3), "(", round(sustain, 3), "%) observations"))
      cat("\n")
      cat(paste0("with effect of ", nu, " with cases with effect of", round(beta_threshhold, 3), " (RIR = ", round(recase, 3), ").\n"))
      cat("\n")
    } else if ((abs(est_eff) < abs(beta_threshhold)) & is.na(eff_thr) == FALSE) {
      cat("TO SUSTAIN:\n", sep = "")
      cat("\n")
