...
test_sensitivity <- function(est_eff,
 ...
 if (est_eff == beta_threshold & index == "RIR") {
  ...
  if (length(to_return) > 1) {
    ...
    return(output_print(n_covariates, est_eff, beta_threshold, bias, sustain, nu, eff_thr, recase, obs_r, critical_r, r_con, itcv, alpha, index, far_bound, sdx, sdy, R2, uncond_rxcv, uncond_rycv))
