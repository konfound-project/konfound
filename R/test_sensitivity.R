...
#' @details Prints a concise summary of konfound output with multiple types
...
# Main function to test sensitivity to be wrapped with pkonfound(), 
# konfound(), and mkonfound()
...
test_sensitivity <- function(est_eff,
                             ...
                             model_object,
                             tested_variable) {
    if (nu != 0) warning("You entered a non-zero null hypothesis about an 
                         effect;this is being interpreted in terms of a partial
                        correlation.Sampling variability is not accounted for.")
    
    ## error message if input is inappropriate
    if (!(std_err > 0)) {stop("Did not run! Standard error needs to
    ...
    # dealing with cases where hypotheses other than whether
    # est_eff differs from 0
    if (nu != 0) {
    ...
    # calculating percentage of effect and number of observations to 
    # sustain or invalidate inference
    ...
    obs_r <- (est_eff / std_err) / sqrt(((n_obs - n_covariates - 3) + 
                                             ((est_eff / std_err)^2)))
    # finding critical r
    ...
    #
    #     rsqXZ = max(0, 1 - ((VarY * (1 - RSQ))) / 
    #                           (VarX * (n_obs - n_covariates - 2) * (sdx * 2)))
    #
    ...
    if (length(to_return) > 1) {
        ...
        output_print(est_eff, beta_threshold, bias, sustain, nu, 
                     recase, obs_r, critical_r, r_con, itcv, alpha, index)
        
        cat("\n")
        message(paste(
            "Print output created by default. Created", length(konfound_output),
            "other forms of output. Use list indexing or run summary() on the 
            output to see how to access."))
