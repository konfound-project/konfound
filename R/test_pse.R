test_pse <- function(est_eff,
    ...
    if (!(sdx > 0)) {stop("Did not run! Standard deviation of x 
                          needs to be greater than zero.")}
    if (!(sdy > 0)) {stop("Did not run! Standard deviation of y 
                          needs to be greater than zero.")}
    ...
    # convert conditional correlations to unconditional 
    # correlations to be used in new regression
    rxcv = rxcvGz * sqrt((1 - rcvz^2) * (1 - rxz^2)) + rxz * rcvz
    rycv = rycvGz * sqrt((1 - rcvz^2) * (1 - rzy^2)) + rzy * rcvz
    
    verify_pse_reg_M3 = verify_reg_Gzcv(n_obs, sdx, sdy, sdz, sdcv, rxy, rxz, 
                                        rzy, rycv, rxcv, rcvz)
    verfiy_pse_manual_thr = verify_manual(rxy, rxz, rxcv, ryz, rycv, 
                                          rzcv, sdy, sdx)
