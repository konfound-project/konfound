...
test_sensitivity_ln <- function(est_eff,
    ...
    if (n_obs <= 0 || n_treat <= 0) {
        stop("Please enter positive integers for sample size and number of 
             treatment group cases.")
    }
    if (n_obs <= n_treat) {
        stop("The total sample size should be larger than the number of 
             treatment group cases.")
    }
  ...
  #t_ob is the t value calculated based on observed estimate and standard error
    ...
    # invalidate_ob is true - the observed result is significant
    # - we are invalidating the observed result
    ...
    # haveimaginary <- F
    # changepi <- F
    # set the default value for whether we need and can adjust pi 
    # (ratio of treatment cases)
    # to remove the imaginary part
    # keyimagin <- (4 + 4 * odds_ratio^2 + odds_ratio *
    #                 (-8 + 4 * n_obs * std_err^2 - n_obs * n_treat * std_err^4 
    #                                                 + n_treat^2 * std_err^4))
    # minimgain <- 4 + 4 * odds_ratio^2 + odds_ratio * (-8 + n_obs * std_err^2 *
    #                                           (4 - 0.25 * n_obs * std_err^2))
    ...
    # n_cnt <- n_obs - n_treat
    #  } else {
    #    stop("Cannot generate a usable contingency table; Please consider 
    #   using the Pearson's chi-squared approach (under development).")
    #  }
    #}
    ...
    # a2, b2, c2, d2 are the second solution for the 4 cells in the 
    # contingency table
    ...
    ### remove the solution if one cell has fewerer than 5 cases 
    ### or negative cells or nan cells
    check1 <- check2 <- TRUE
    if (!(n_cnt >= a1 && a1 >= 5 && n_cnt >= b1 && b1 >= 5 && 
          n_treat >= c1 && c1 >= 5 && n_treat >= d1 && d1 >= 5)
        || is.nan(a1) || is.nan(b1) || is.nan(c1) || is.nan(d1)) {
        check1 <- FALSE
    }
    
    if (!(n_cnt >= a2 && a2 >= 5 && n_cnt >= b2 && b2 >= 5 && 
          n_treat >= c2 && c2 >= 5 && n_treat >= d2 && d2 >= 5)
    ...
    if (!final_solution$needtworows & final_solution$final_switch > 1) {
        conclusion1 <- paste(
            change, sprintf("you would need to replace %d", 
                            RIR), RIRway, "cases")
        
        if (replace == "control") {
            conclusion1a <- sprintf(
                "with cases for which the probability of failure in the
                control group applies (RIR = %d).", RIR)
        } else {
            conclusion1a <- sprintf(
                "with cases for which the probability of failure in the entire 
                sample applies (RIR = %d).", RIR)
        }
        
        conclusion1b <- paste(
            sprintf("This is equivalent to transferring %d", 
        ...
        if (replace == "control") {
            conclusion1a <- sprintf("with cases for which the probability of 
                                    failure in the control group applies 
                                    (RIR = %d).", RIR)
        } else {
            conclusion1a <- sprintf("with cases for which the probability of 
                                    failure in the entire sample applies
                                    (RIR = %d).", RIR)
        }
        ...
        conclusion1 <- paste(
            change, c(
                "only transferring cases from"), transferway, "is not enough.")
        
        conclusion1b <- paste(sprintf("We also need to transfer %d cases from", 
                                      final_solution$final_extra),
                              transferway_extra, c("as shown, from the 
                                                   User-entered Table to the 
                                                   Transfer Table."))
        
        conclusion1c <- paste(sprintf("This means we need to replace %d of", 
                                      RIR), RIRway, 
                              sprintf("with null hypothesis cases; and 
                                      replace %d", RIR_extra), RIRway_extra, 
                              c("with null hypothesis cases to change 
                                the inference.")
        )
    }
    ...
    conclusion2 <- sprintf(
        ...
        final_solution$t_start
    )
    
    conclusion3 <- sprintf(
        "For the Transfer Table, we have an estimate of %.3f, 
        with a SE of %.3f and a t-ratio of %.3f.",
        final_solution$est_eff_final, final_solution$std_err_final, 
        final_solution$t_final
    )
    
    notice <- "Note: Values have been rounded to the nearest integer."
    noticeb <- 
        "This may cause a little change to the estimated 
         effect for the Implied Table."
    
    if (changeSE) {
        notice_SE <- sprintf(
            "In order to generate a usable implied contingency table, we 
            increased the standard error to %.3f (the original one is %.3f).",
    ...
    #                Implied_Table = final_solution$table_start, notice, 
