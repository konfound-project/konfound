#' Draw figures for change in effect size as a function of switching 
#' or replacing outcomes 
#' @description This function returns two plots for change in effect size 
#' as a function of switching or replacing outcomes, one for all
#' possibilities (switching), another zoomed in the area for positive RIR  
#' @param a cell is the number of cases in the control group showing 
#' unsuccessful results
#' @param b cell is the number of cases in the control group showing 
#' successful results
#' @param c cell is the number of cases in the treatment group showing 
#' unsuccessful results
#' @param d cell is the number of cases in the treatment group showing 
#' successful results
#' @param thr_p the p-value threshold used to evaluate statistical 
#' significance, with the default of 0.05
#' @param switch_trm whether switching the two cells in the treatment row 
#' or the two cells in the control row, with the default of the treatment row
#' @param test whether using Fisher's Exact Test (default) or chi-square test
#' @param replace whether using entire sample or the control group to calculate 
#' the base rate, with the default of the control group  
#' @importFrom stats chisq.test
#' @return prints 2 figures for how number of hypothetical cases switched 
...
#' tkonfound_fig(35, 17, 17, 38, thr_p = 0.01, switch_trm = TRUE, 
#' test = "chisq")
#' tkonfound_fig(35, 17, 17, 38, thr_p = 0.01, switch_trm = TRUE, 
#' test = "chisq", replace = "entire")
#' 
#' @export
#' 
...
tkonfound_fig <- function(a, b, c, d, thr_p = 0.05, switch_trm = T,
    ...
    if (test == "fisher"){
        ...
        for (i in 1:(n_obs-3)){
            meta$logodds[i] <- log(
                fisher_oddsratio(meta$a[i], meta$b[i], meta$c[i], meta$d[i]))
        }
    }
    ...
    meta$sig <- ifelse(meta$a==table_final[1,1] & 
                           meta$b==table_final[1,2] & meta$c==table_final[2,1] &
                           meta$d==table_final[2,2],1,0)
    ...
    if (meta[meta$sig==1,]$logodds > 0){
        if (dcroddsratio_ob){#from sig to not sig by decreasing odds ratio 
            ...
            pos_thr <- (meta[meta$switch==posinsig,]$logodds+
                            meta[meta$switch==(posinsig+1),]$logodds)/2
            pos_thr_pdif <- (meta[meta$switch==posinsig,]$pdif+
                                 meta[meta$switch==(posinsig+1),]$pdif)/2
            zoom_lower <- -(posinsig + 2)
            meta$sigpoint <- ifelse(meta$switch==(posinsig+1),
                                    "positive","other")
        } else {#from not sig to sig by increasing positive effect
            possig <- meta[meta$sig==1,]$switch
            pos_thr <- (meta[meta$switch==possig,]$logodds+
                            meta[meta$switch==(possig-1),]$logodds)/2
            pos_thr_pdif <- (meta[meta$switch==possig,]$pdif+
                                 meta[meta$switch==(possig-1),]$pdif)/2
            zoom_lower <- -(possig + 2)
        ...
        j <- 1
        if (test == "chisq"){
            while (chisq_p(temp1$a[j], temp1$b[j],
                           temp1$c[j], temp1$d[j])>thr_p){
                j <- j+1
            }
        }
        if (test == "fisher"){
            while (fisher_p(temp1$a[j], temp1$b[j], 
                            temp1$c[j], temp1$d[j])>thr_p){
                j <- j+1
            }
        }
        ...
        meta$sigpoint <- ifelse(meta$switch==temp1$switch[j],
                                "negative",meta$sigpoint)
    }
    
    if (meta[meta$sig==1,]$logodds < 0){
        if (dcroddsratio_ob){ # from not sig to sig by decreasing odds ratio
            negsig <- meta[meta$sig==1,]$switch
            neg_thr <- (meta[meta$switch==negsig,]$logodds+
                            meta[meta$switch==(negsig+1),]$logodds)/2
            neg_thr_pdif <- (meta[meta$switch==negsig,]$pdif+
                                 meta[meta$switch==(negsig+1),]$pdif)/2
            zoom_upper <- -(negsig - 4)
    ...
    # that is smaller than 1 
            ...
            neg_thr_pdif <- (meta[meta$switch==neginsig,]$pdif+
                                 meta[meta$switch==(neginsig-1),]$pdif)/2 
    ...
    fillcol <-c("current"="white",
                "positive"="green4","negative"="red","other"="white") 
    pointshape <- c("current"=15,"other"=21)
    
    if (switch_trm && dcroddsratio_ob) {
        meta$RIR <- ceiling(meta$switch/((a+c)/n_obs))*(replace=="entire") + 
            ceiling(meta$switch/(a/(a+b)))*(1-(replace=="entire"))
    }
    if (switch_trm && !dcroddsratio_ob) {
        meta$RIR <- ceiling(meta$switch/((b+d)/n_obs))*(replace=="entire") + 
            ceiling(meta$switch/(b/(a+b)))*(1-(replace=="entire"))
    }
    if (!switch_trm && dcroddsratio_ob) {
        meta$RIR <- ceiling(meta$switch/((b+d)/n_obs))*(replace=="entire") + 
            ceiling(meta$switch/(b/(a+b)))*(1-(replace=="entire"))
    ...
    fig1 <- ggplot2::ggplot(meta, ggplot2::aes_string(x="RIR", y="pdif"))+
        ...
        ggplot2::geom_point(ggplot2::aes_string(
            y="pdif", shape = "current",fill = "sigpoint"))+
        ...
        ggplot2::geom_hline(
            yintercept = pos_thr_pdif, 
            linetype = "dashed", color="green4", size = 1)+
        ggplot2::geom_hline(
            yintercept = neg_thr_pdif,
            linetype = "dashed", color="red", size = 1)+
        ggplot2::scale_y_continuous(
            name="Difference in probability of 
            successful outcome (treatment - control)")+
        ggplot2::scale_x_continuous(name="RIR (Fragility)", 
                                    breaks= c(meta[meta$switch==0,]$RIR, meta[
                                        meta$sigpoint=="negative",]$RIR,
                                              meta[meta$sigpoint=="positive",
                                                   ]$RIR),
                                    labels= c(meta[meta$switch==0,]$xaxis, 
                                              meta[meta$sigpoint=="negative",
                                                   ]$xaxis,
                                              meta[meta$sigpoint=="positive",
                                                   ]$xaxis)) +
    ...
    zoom$label <- ifelse(zoom$sigpoint=="positive", 
                         paste("sig pos:RIR=", zoom[zoom$sigpoint=="positive",
                                                    ]$RIR),NA)
    zoom$label <- ifelse(zoom$sigpoint=="negative", 
                         paste("sig neg:RIR=", zoom[zoom$sigpoint=="negative",
                                                    ]$RIR),zoom$label)
    ...
    fig2 <- ggplot2::ggplot(zoom, ggplot2::aes_string(x="RIR",y="pdif"))+
        ggplot2::geom_line(ggplot2::aes_string(y="pdif"), size = 1) +
        ggplot2::geom_point(ggplot2::aes_string(y="pdif", shape = "current",fill
                                                = "sigpoint"), 
                            size = 1)+
        ggrepel::geom_label_repel(ggplot2::aes_string(label="label"))+
        ggplot2::scale_fill_manual(values=fillcol)+
        ggplot2::scale_shape_manual(values=pointshape)+
        ggplot2::scale_y_continuous(name="Difference in probability of 
                                    successful outcome (treatment - control)")+
        ggplot2::scale_x_continuous(name="RIR (Fragility)", 
                                    breaks= c(zoom$RIR[1], zoom$RIR[
                                        as.integer(length(zoom$RIR)/2)], 
                                              zoom$RIR[as.integer(
                                                  length(zoom$RIR))]),
                                    labels= c(zoom$xaxis[1], zoom$xaxis[
                                        as.integer(length(zoom$RIR)/2)],
                                              zoom$xaxis[as.integer(
                                                  length(zoom$RIR))])) +
    ...
    if (pos_thr_pdif <= max(zoom$pdif) && pos_thr_pdif >= min(zoom$pdif)) {
        fig2 <- fig2 + ggplot2::geom_hline(yintercept = pos_thr_pdif, linetype
                                           = "dashed", color="green4", size = 1)
    }
    
    if (neg_thr_pdif <= max(zoom$pdif) && neg_thr_pdif >= min(zoom$pdif)) {
        fig2 <- fig2 + ggplot2::geom_hline(yintercept = neg_thr_pdif, linetype 
                                           = "dashed", color="red", size = 1)
    }
    
    ###plot figure 3 RIS% as sample size gets larger, 
    ### using t statistic as the criterion
    # if (plt3 == TRUE) {
    #  meta3 <- data.frame(matrix(ncol = 7, nrow = 11))
    #  colnames(meta3) <- c("a", "b", "c", "d", "nobs", "RIS", "RISperc_total", 
    #  "RISperc_D") 
    #  for (i in 1:11){
    ...
    #    meta3$RIS[i] <- RIS_i <- getswitch(table_i, thr_t_i, 
    #   switch_trm, size)$final_switch + getswitch(table_i, thr_t_i, 
    #   switch_trm, size)$final_extra
