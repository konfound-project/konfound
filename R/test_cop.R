...
test_cop <- function(est_eff, # unstandardized
    ...
    #          sdx = 2, sdy = 6, R2 = .7, eff_thr = 0, 
    #           FR2max = .8, to_return = "short")
    ...
    if (!(std_err > 0)) {stop("Did not run! Standard error needs to be greater
                              than zero.")}
    if (!(sdx > 0)) {stop("Did not run! Standard deviation of x needs to 
                          be greater than zero.")}
    if (!(sdy > 0)) {stop("Did not run! Standard deviation of y needs to 
                          be greater than zero.")}
    ...
    if (!(R2 < FR2max)) {stop("Did not run! R2 Max needs 
                              to be greater than R2.")}
    if (!(FR2max < 1)) {stop("Did not run! R2 Max needs to be less than 1.")}
    if (!(1-((sdy^2/sdx^2)*(1-R2)/((df+1) * std_err^2))>0)) 
        {stop("Did not run! Entered values produced Rxz^2 <=0, consider 
              adding more significant digits to your entered values.")
        }
    # an indicator of whether the use specified est_eff is 
    # negative, 1 is yes negative
    negest <- 0 
    if (est_eff < 0) {
        est_eff <- abs(est_eff)
        negest <- 1
    ...
    delta_star = cal_delta_star(FR2max,
                                R2, 
                                R2_uncond, 
                                est_eff, 
                                eff_thr, 
    ...
    ## calculate the exact/true rcvx, rcvy, 
    ## delta (updated version that does not need rxy)
    ...
    ## exact_result = cal_delta_exact
    ## (ryx, ryz, rxz, beta_thr, FR2max, R2, sdx, sdz)
    ...
    fTable <- matrix(c(R2_M1, R2_M2, R2_M3, R2_M3_oster, 
                       # R2 for three reg models
...
# NA, eff_z_M2 / se_z_M2, eff_z_M3 / se_z_M3, eff_z_M3_oster / se_z_M3_oster, 
    ...
    fig <- ggplot2::ggplot(figTable, ggplot2::aes(x = ModelLabel)) +
        ...
        ggplot2::geom_point(
            ggplot2::aes(
                y = R2/scale), color = "#7CAE00", shape = 18, size = 4) + 
        # scale_linetype_manual(values=c("solid", "dotted")) +
        ggplot2::geom_line(
            ggplot2::aes(
                y = R2/scale, group = cat), 
            linetype = "solid", color = "#7CAE00") + 
        ggplot2::geom_line(
            ggplot2::aes(
                y = coef_X, group = cat, linetype = cat), color = "blue") + 
    ...
    if (est_eff < 0) {
        critical_t <- stats::qt(1 - (alpha / tails), 
                                n_obs - n_covariates - 2) * -1
    } else {
    ...
    if (to_return == "raw_output") {
        if (negest == 1) {
            cat("Using the absolute value of the estimated
                effect, results can be interpreted by symmetry.")
            cat("\n")
        }
    ...
    if (to_return == "print") {
        ...
        cat(sprintf("delta* is %.3f (assuming no covariates in the baseline 
                    model M1), the exact delta is %.3f, with a bias of %.3f%%", 
                    delta_star, delta_exact, delta_pctbias))
        cat("\n")
        cat(sprintf("With delta*, the coefficient in the final model will be
                    %.3f. With the exact delta, the coefficient will be %.3f.",  
                    eff_x_M3_oster,eff_x_M3))
        cat("\n")
        cat("Use to_return = raw_ouput to see more specific results and graphic
        ...
        cat("\n")
        cat("If the replacement cases have a fixed value, then RIR =", 
            cond_RIR_fixedY)
        cat("\n")
        cat("If the replacement cases follow a null distribution, then RIR =", 
            cond_RIR_null)
        cat("\n")
        cat("If the replacement cases satisfy rxy|Z = 0, then RIR =", 
            cond_RIR_rxyz)
        cat("\n")
        cat("\n")
    }
