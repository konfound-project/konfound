...
konfound_lm <- function(model_object, tested_variable_string, test_all, alpha, tails, index, to_return) {
  ...
  if (test_all == FALSE | n_covariates == 1) {
    ...
    message("The mkonfound command is used which evaluates RIR in terms of a correlation-based approach to support comparison among the robustness of the inferences across the covariates. ITCV is for conditional correlations. To generate unconditional ITCV, use the pkonfound command for each predictor separately.")
