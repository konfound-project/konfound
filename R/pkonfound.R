...
#' @description For published studies, this command calculates 
...
#' @param est_eff the estimated effect (such as an unstandardized beta 
#' coefficient or a group mean difference)
#' @param std_err the standard error of the estimate of the unstandardized 
...
#' @param tails integer whether hypothesis testing is one-tailed (1) or 
#' two-tailed (2; defaults to 2)
#' @param index whether output is RIR or IT (impact threshold); 
#' defaults to "RIR"
#' @param nu what hypothesis to be tested; defaults to testing whether 
#' est_eff is significantly different from 0
#' @param model_type the type of model being estimated; defaults to "ols" for 
#' a linear regression model; the other option is "logistic"
#' @param n_treat the number of cases associated with the treatment condition; 
#' applicable only when model_type = "logistic"
#' @param switch_trm whether to switch the treatment and control cases; defaults
#'  to FALSE; applicable only when model_type = "logistic"
#' @param a cell is the number of cases in the control group showing 
#' unsuccessful results
#' @param b cell is the number of cases in the control group showing successful 
#' results
...
#' successful results
#' @param two_by_two_table table that is a matrix or can be coerced to one 
#' (data.frame, tibble, tribble) from which the a, b, c, and d arguments can 
#' be extracted
#' @param test whether using Fisher's Exact Test or A chi-square test; defaults 
#' to Fisher's Exact Test
#' @param replace whether using entire sample or the control group to calculate
#'  the base rate; default is the control group
...
#' @param FR2max the largest R2, or R2max, in the final model with unobserved 
#' confounder 
#' @param FR2max_multiplier the multiplier of R2 to get R2max, 
#' default is set to 1.3
#' @param to_return whether to return a data.frame 
#' (by specifying this argument to equal "raw_output" for use in other analyses)
#'  or a plot ("plot"); default is to print ("print") the output to the console;
#'   can specify a vector of output to return
  ...
  if ("table" %in% to_return) stop("a table can only be 
   ...
   if (index == "COP") {
    ...
    warning("For a logistic regression, the log-odds coefficients 
            are used for the pkonfound() calculations. To carry out an 
            analysis using average marginal effects, you can use the 
            konfound() function with the results of a model estimated in R.")
