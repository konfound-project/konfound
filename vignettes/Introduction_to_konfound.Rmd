...
We can also output multiple forms of output at once:
...
We can then type each of those, i.e.:
```{r}
...
As with models fit with `lm()`, multiple forms of output can be specified with the `to_return` argument to `konfound()`, i.e. `konfound(m2, age, to_return = c("raw_output", "corr_plot", "thresh_plot"))`.
