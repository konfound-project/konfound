...
```{r}
model_output <- pkonfound(2, .4, 200, 3, to_return = c("df", "thresh_plot", "corr_plot"))
...
Here, `df` represents output from a number of past studies. You can read a data file with results from 30 (hypothetical) studies. Note that this example data only had `t` and `df` values, and so sensible values for the number of observations and the [STILL WORKING ON THIS].
