...
test_that("test_cop outputs the correct language and values when to_return is 'print'", {
    ...
    expected_lines <- c(
        ...
        "proportionality (delta_Correlation) along with Oster's delta*.",
        "The correlation-based COP does not depend on the specification",
        "of a baseline model.",
        "",
        "COP ",
        ...
        "to satisfy the specified conditions (eff_thr = 0.3 and FR2max = 0.30) is",
        "r_xcv * r_ycv = 0.5482 * 0.4495 = 0.2464, which is 225% of the impact of the",
        "observed covariates (R_XZ * R_YZ = 0.4677 * 0.2339 = 0.1094).",
        "",
        "Through the product r_xcv * r_ycv, Frank's impact accounts for the relationship",
        "of the covariate to the outcome (Y) as well as the focal predictor (X).",
        "See the konfound command with index = \"IT\" for details of the Impact Threshold",
        "for a Confounding Variable (ITCV).",
        "",
        "Threshold Based on Statistical Significance ",
        "Using alpha = 0.05 and df = 96 (so critical r = 0.1986),",
        "the delta threshold for statistical significance is 1.578.",
        "This requires a CV (omitted confounder) with partial correlations",
        "r_xcv|z ~ 0.8350 (between X and CV given Z) and",
        "r_ycv|z ~ 0.4260 (between Y and CV given Z).",
        "",
        "Using the delta threshold for statistical significance",
        "and the corresponding partial correlations,",
        "the coefficient of X in the final model will be 0.3591",
        "with standard error of 0.1809 with t-ratio of 1.9850",
        "and the final R2 will be 0.257.",
        "",
